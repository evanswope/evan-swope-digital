...
You must judge this on TWO criteria: 1. BASE ITEM (CRITICAL): Does the image visually contain the exact "Base Item Wanted" requested by the customer? If the image is messy, blurry, or ambiguous, you MUST TRUST the player's prompt and assume the base item is present (approved: true). Only reject it (approved: false) if you CLEARLY see a completely different object (e.g., they asked for eggs, but the image is perfectly clearly a car or a dog). If the image looks even remotely like the requested base item, or is just a weird ambiguous blob that could be the item, you MUST ACCEPT IT (approved: true). Be extremely lenient.
