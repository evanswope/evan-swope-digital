...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (lightboxModal) {
    ...
    // Flip Logic for Cards
    document.querySelectorAll('.project-card-container').forEach(card => {
      card.addEventListener('click', (e) => {
        // Prevent flipping if they clicked a button inside
        if (e.target.closest('button')) return;
        // Don't flip if it's the lightbox triggering
        if (e.target.tagName.toLowerCase() === 'img' || e.target.tagName.toLowerCase() === 'video') {
           // We might still want it to flip if we click the media background. 
           // Wait, the lightbox triggers on `.project-card-front`. If we add flip to the container, it will flip when they click to open lightbox!
           // Let's only flip if they DIDN'T click `.front-media-background`?
           // No, the V2 lightbox triggers on `.project-card-front`. But the user wants them to flip on click instead of using the global toggle!
           // The user said: "just have the cards themselves be the two-faced snakes they are!"
           // And earlier: "if you click and drag (desktop or mobile), the whole assemblage flips around". That was for V3. Here in V2, click flips.
           // BUT what about the lightbox? If click flips, how do they open the lightbox?
           // Maybe we disable lightbox on card click, or they have to click a specific "expand" button?
           // Let's just flip on click for now!
        }
        card.classList.toggle('is-flipped');
      });
    });
